...
openssl req -x509 -new -nodes -key ca.key -sha256 -subj "/CN=${COMMON_NAME}" -days 825 -out ca.crt -extensions v3_ca -config openssl-with-ca.cnf
...
openssl x509 -req -in tls.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out tls.crt -days 825 -sha256 -extfile openssl-with-ca.cnf -extensions san_ext
