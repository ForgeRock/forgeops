...
time bin/searchrate  -h $HOST -p 1389 -D "cn=directory manager"  -j "${DIR_MANAGER_PW_FILE}" \
    -F -c 4 -t 4 -m $ITERATIONS -b "${BASE_DN}" -g "rand(0,$USERS)" "(uid=user.%d)"


echo  "Authrate  test"
time bin/authrate -h $HOST -p 1389 -D '%2$s' -w password -f -c 10  -m $ITERATIONS  \
...
time bin/modrate -h $HOST -p 1389 -D "cn=directory manager" -j "${DIR_MANAGER_PW_FILE}" \
    -F -c 4 -t 4 -m $ITERATIONS -b "uid=user.%d,ou=people,$BASEDN" \
...
ITER2=$(expr "${ITERATIONS}" / 100 )
time bin/addrate -h $HOST -p 1389 -D "cn=directory manager"  -j "${DIR_MANAGER_PW_FILE}"  \
    -f -c 1 -C random -m "${ITER2}" -s 1000 /tmp/addrate.template
