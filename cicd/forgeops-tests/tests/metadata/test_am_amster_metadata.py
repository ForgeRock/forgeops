...
class TestAMAmsterMetadata(object):
    ...
    def test_am_amster_versions(self):
        ...
        am_version = TestAMAmsterMetadata.representative_am_pod.get_version()
        amster_version = TestAMAmsterMetadata.representative_amster_pod.get_version()

        assert (am_version['VERSION'] == amster_version['VERSION']), \
            'AM version [%s] is not the same as Amster [%s]' % (am_version['VERSION'], amster_version['VERSION'])
        assert (am_version['REVISION'] == amster_version['REVISION']),\
            'AM version [%s] is not the same as Amster [%s]' % (am_version['REVISION'], amster_version['REVISION'])
