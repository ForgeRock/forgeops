...
upgrade_config(){
	$script_dir/am-config-upgrader  $DOCKER_ROOT/am/config-profiles/exported config/am-upgrader-rules
}
	...
	for p in "${COMPONENTS[@]}"; do
	   ...
	   case $p in
		idm)
			...
			rm -fr  "$DOCKER_ROOT/idm/config-profiles/exported/conf"
			pod=$(kubectl get pod -l app=idm -o jsonpath='{.items[0].metadata.name}')
			kubectl cp -c openidm $pod:/opt/openidm/conf "$DOCKER_ROOT/idm/config-profiles/exported/conf"
			printf "\nIDM configuration files have been exported to ${DOCKER_ROOT}/idm/config-profiles/exported/config."
			;;
		amster)
			rm -fr "$DOCKER_ROOT/amster/config-profiles/exported/config"
			mkdir -p "$DOCKER_ROOT/amster/config-profiles/exported/config"
			echo "$script_dir/amster" export "$DOCKER_ROOT/amster/config-profiles/exported/config"
			"$script_dir/amster" export "$DOCKER_ROOT/amster/config-profiles/exported/config"
			;;
		am)
			...
			kubectl exec $pod -c openam -- /home/forgerock/export-diff.sh - | tar xf -  -C "$DOCKER_ROOT"/am/config-profiles/exported || {
				echo ""
				echo "No changes were made to the AM configuration. Exiting."
				exit 0
			}

			printf "\nAM configuration files have been exported to ${DOCKER_ROOT}/am/config-profiles/exported/config."
...
save_config()
{
	...
	for p in "${COMPONENTS[@]}"; do
		...
		idm)
			...
			cp -R "$DOCKER_ROOT/idm/config-profiles/exported/conf"  "$PROFILE_ROOT/idm"
			;;
		amster)
			...
			echo ""
			find "$DOCKER_ROOT/amster/config-profiles/exported/config" -name "*.json" \
			...
			CLIENT_ROOT="$DOCKER_ROOT/amster/config-profiles/exported/config/realms/root/OAuth2Clients"
			IGAGENT_ROOT="$DOCKER_ROOT/amster/config-profiles/exported/config/realms/root/IdentityGatewayAgents"
			...
			#****** COPY FIXED FILES ******#
			cp -R "$DOCKER_ROOT/amster/config-profiles/exported/config"  "$PROFILE_ROOT/amster"
		...
		*)
			...
			#****** COPY FIXED FILES ******#
			cp -R "$DOCKER_ROOT/am/config-profiles/exported/config"  "$PROFILE_ROOT/am"
		esac
	done
}
