...
# This script assumes that oc and helm are available, and have been configured
...
create_namespace()
{
    ...
    if $(oc get namespace ${NAMESPACE} > /dev/null 2>&1); then
        ...
        if [ $? -ne 0 ]; then
            echo "Non-zero return by oc.  Is your context correct? Exiting!"
            exit 1
        fi
    fi
}
...
import_check()
{
    ...
    AMSTER_POD_NAME=$(oc -n=${NAMESPACE} get pods --selector=component=amster \
...
restart_am()
{
    OPENAM_POD_NAME=$(oc -n=${NAMESPACE} get pods --selector=app=openam \
        -o jsonpath='{.items[*].metadata.name}')
    echo "=> Deleting \"${OPENAM_POD_NAME}\" to restart and read newly imported configuration"
    oc delete pod $OPENAM_POD_NAME --namespace=${NAMESPACE}
    if [ $? -ne 0 ]; then
...
scale_am()
{
    ...
    DEPNAME=$(oc get deployment -l app=openam -o name)
    oc scale --replicas=2 ${DEPNAME} || true
    if [ $? -ne 0 ]; then
...
scale_idm()
{
    ...
    DEPNAME=$(oc get statefulset -l app=openidm -o name)
    oc scale --replicas=2 ${DEPNAME} || true
    if [ $? -ne 0 ]; then
...
deploy_hpa()
{
    ...
    oc apply -f ${CFGDIR}/hpa.yaml || true
    if [ $? -ne 0 ]; then
...
echo "   $ oc exec -it <podname> scripts/schedule-backup.sh"
