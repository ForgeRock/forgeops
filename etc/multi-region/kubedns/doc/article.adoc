...
[[docker-config-DS,Docker configuration for DS]]
=== a. Docker configuration for DS

To fulfill xref:introduction-server-id[] and configure a unique server ID inside the topology,
this makes the following change to +docker-entrypoint.sh+:

* the server ID is computed using the hostname and the first subdomain which corresponds to the service name defined
in a current cluster (Ex. for FQDN +*ds-cts-0.ds-cts-us*.multi-region.svc.cluster.local+ the server ID will be
+*ds-cts-0_ds-cts-us*+)

****
docker-entrypoint.sh https://github.com/ForgeRock/forgeops/tree/master/docker/7.0/ds/scripts/docker-entrypoint.sh[(latest version)]
....
export DS_SERVER_ID=${DS_SERVER_ID:-$(hostname -f | awk -F. '{printf("%s_%s", $1, $2)}')}
....
****

Also, this script is added as an execution step to DS-CTS and DS-IDREPO ++Dockerfile++s.

****
Dockerfile https://github.com/ForgeRock/forgeops/tree/master/docker/7.0/ds/cts/Dockerfile[(DS_CTS latest version)]
https://github.com/ForgeRock/forgeops/tree/master/docker/7.0/ds/idrepo/Dockerfile[(DS_IDREPO latest version)]
....
...
=== b. Prepare Kustomize definitions
