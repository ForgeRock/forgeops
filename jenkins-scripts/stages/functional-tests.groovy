...
void runStage() {
    ...
    dockerUtils.insideGoogleCloudImage(dockerfilePath: 'docker/google-cloud', getDockerfile: true) {
        dir('guillotine') {
            ...
            // Configure Guillotine to run functional tests (all test suites with RELEASE keyword)
            sh("./configure.py runtime --forgeops-branch-name ${branchName} --keywords RELEASE")
            try {
                // Run the tests
                sh("./run.py")
            } finally {
